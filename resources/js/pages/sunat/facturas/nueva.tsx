...
const DETRACCIONES = [
    ...
    {
        codigo: '010',
        descripcion: 'Residuos, subproductos, desechos',
        porcentaje: 15,
    },
    {
        codigo: '012',
    ...
    {
        codigo: '036',
        descripcion: 'Oro y demás minerales metálicos exonerados',
        porcentaje: 1.5,
    },
    {
        codigo: '037',
...
export default function NuevaFactura({
    ...
    // ── Cabecera ──
    const [tipoDoc, setTipoDoc] = useState<'01' | '03'>(
        tipo_inicial === 'boleta' ? '03' : '01',
    );
    const series = tipoDoc === '01' ? series_factura : series_boleta;
    const [serie, setSerie] = useState(
        series[0]?.serie ?? (tipoDoc === '01' ? 'F001' : 'B001'),
    );
    ...
    const [clienteTipoDoc, setClienteTipoDoc] = useState(
        cotizacion?.cliente.tipo_documento ?? '6',
    );
    ...
    async function lookupRuc(numero: string) {
        ...
        try {
            const res = await fetch(
                `/sunat/buscar-ruc?numero=${encodeURIComponent(trimmed)}`,
                {
                    headers: { Accept: 'application/json' },
                },
            );
            ...
            if (data.tipo_documento || trimmed.length === 11)
                setClienteTipoDoc(trimmed.length === 11 ? '6' : '1');
        } catch {
    ...
    const totals = useMemo(() => {
        ...
        const detMonto =
            detEnabled && detPct > 0 ? totalComprobante * (detPct / 100) : 0;
    ...
    function validate() {
        ...
        if (items.some((it) => it.precio_unitario <= 0))
            e.items_precio = 'El precio unitario debe ser mayor a 0.';
        if (formaPago === 'Credito' && !fechaVencimiento)
            e.vencimiento = 'Ingresa la fecha de vencimiento.';
        if (detEnabled && !detCodigo)
            e.det_codigo = 'Selecciona el bien o servicio sujeto a detracción.';
        if (detEnabled && !detCuenta)
            e.det_cuenta = 'La cuenta del Banco de la Nación es requerida.';
        setErrors(e);
    ...
    function submit(enviarAuto: boolean) {
        ...
        const payload = {
            ...
            fecha_vencimiento:
                formaPago === 'Credito' ? fechaVencimiento : null,
            cuotas:
                formaPago === 'Credito'
                    ? [
                          {
                ...
                {cotizacion && (
                    <div className="mb-4 flex items-center gap-3 rounded-xl border border-primary/30 bg-primary/5 px-4 py-3 text-sm">
                        ...
                        <span>
                            ...
                            <strong>{cotizacion.numero}</strong>. Revisa y emite
                            cuando estés listo.
                        </span>
                    </div>
                )}
                    ...
                    <div className="flex flex-col gap-5">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <div className="flex gap-1.5">
                                            ...
                                            <button
                                                type="button"
                                                onClick={() =>
                                                    lookupRuc(clienteNumDoc)
                                                }
                                                disabled={
                                                    lookingUpRuc ||
                                                    clienteNumDoc.length < 8
                                                }
                        ...
                        <section className="shadow-soft rounded-2xl border border-border bg-card">
                            <div className="flex items-center justify-between border-b border-border/60 px-5 py-3.5">
                                <span className="text-sm font-semibold text-foreground">
                                    Descripción de bienes vendidos / servicios
                                    prestados
                                </span>
                                <button
                                    type="button"
                                    ...
                                    <div className="p-5">
                                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                            ...
                                            <div className="flex flex-col gap-1.5 sm:col-span-2 lg:col-span-3">
                                                <Label className="text-xs font-medium text-muted-foreground">
                                                    Bien o servicio sujeto a
                                                    detracción (Catálogo 54)
                                                </Label>
                                                <select
                                                    value={detCodigo}
                                                    onChange={(e) =>
                                        ...
                                        {detCodigo && (
                                            <div className="mt-4 rounded-xl border border-primary/20 bg-primary/5 p-4">
                                                <p className="mb-2 text-[11px] font-semibold tracking-wider text-primary uppercase">
                                                    Datos de detracción
                                                    calculados
                                                </p>
                            ...
                            <div className="p-4 font-mono text-xs">
                                            ...
                                            {items
                                                ...
                                                .map((it, i) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <p className="leading-tight font-medium uppercase">
                                                                {it.descripcion}
                                                            </p>
                                ...
                                {detEnabled &&
                                    ...
                                    totals.detMonto > 0 && (
                                        <div className="mb-3 rounded-lg border border-amber-300 bg-amber-50 p-2.5 dark:border-amber-700/50 dark:bg-amber-900/20">
                                            <p className="mb-1.5 text-[10px] font-bold text-amber-800 uppercase dark:text-amber-300">
                                                Datos de detracción
                                            </p>
                                ...
                                {formaPago === 'Credito' && (
                                    <div className="mb-3 rounded-lg border border-blue-300 bg-blue-50 p-2.5 dark:border-blue-700/50 dark:bg-blue-900/20">
                                        <p className="mb-1.5 text-[10px] font-bold text-blue-800 uppercase dark:text-blue-300">
                                ...
                                <div className="rounded-xl bg-primary px-4 py-3 text-center">
                                    <p className="text-[10px] font-bold tracking-wide text-primary-foreground/80 uppercase">
                                ...
                                <div className="mt-3 flex items-center justify-center gap-3 border-t border-dashed border-border/60 pt-3">
                                    ...
                                    <div className="flex-1 text-[9px] leading-relaxed text-muted-foreground">
                                        <p>
